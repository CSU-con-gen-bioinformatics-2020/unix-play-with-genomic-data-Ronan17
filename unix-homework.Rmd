...
```
drwxr-xr-x 1 acer 197121      0 Jan 29 07:15 .
drwxr-xr-x 1 acer 197121      0 Jan 29 07:15 ..
drwxr-xr-x 1 acer 197121      0 Jan 29 07:15 .Rproj.user
drwxr-xr-x 1 acer 197121      0 Jan 29 07:15 .git
-rw-r--r-- 1 acer 197121    134 Jan 29 07:15 .gitignore
```
...
```
cd fastq/
cd fastq/DPCh_plate1_
cd fastq/DPCh_plate1_A
cd fastq/DPCh_plate1_A0
cd fastq/DPCh_plate1_A06
cd fastq/DPCh_plate1_A06_S6.R
fastq/DPCh_plate1_A06_S6.R1
cd fastq/DPCh_plate1_A06_S6.R1.fq.gz
```
...
```
To make sure that we delete the right file (and not another one with a similar name) and also see if we don't forgot anything in the directory if it's a directory.
```

# If you had just typed `rm -r bad directory name with spaces` what would Unix have tried to remove?

```
It would have tried to remove bad, directory, name, with and spaces directories.
```
...
```{bash}
ls -a it_aint_here 2> my_bad.txt
cat my_bad.txt
rm my_bad.txt
```
...
```
They have almost the same description, the only change is the two numbers before the "/2 or /1". And they are actually file for Read 1 and Read 2, that's why the description of each line is almost indentical between those two file.
```
...
```{bash}
zcat fastq/*R1* > R1.txt
zcat fastq/*R2* > R2.txt
wc R1.txt
wc R2.txt
rm R1.txt R2.txt
```
